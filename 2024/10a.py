...
def main():
    ...
    peak_sets = (set(_yield_peaks(topo_map, y=y, x=x)) for y, x in trailheads)
    res = sum(len(peaks) for peaks in peak_sets)
    print(res)
...
def _yield_peaks(topo_map, *, y, x):
    cell_value = topo_map[y][x]
    if cell_value == _MAX_CELL_VALUE:
        yield (y, x)
    else:
        yield from itertools.chain.from_iterable(
            _yield_peaks(topo_map, y=next_y, x=next_x)
