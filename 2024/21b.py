...
def main():
    ...
    arrowpad_command_lens = [
        _arrowpad_command_len_for_numpad_code(code, target_arrowpad_depth=25)
        for code in numpad_codes
    ]

    complexity_scores = [
        cmd * _numpad_code_int(code)
        for cmd, code in zip(arrowpad_command_lens, numpad_codes)
    ]
...
def _arrowpad_command_len_for_numpad_code(code, target_arrowpad_depth):
    return sum(
        _best_numpad_move_len(start, end, target_arrowpad_depth)
        for start, end in zip(f"A{code}", code)
    )


def _arrowpad_command_len_for_arrowpad_command(command, target_depth, depth=0):
    if depth == target_depth:
        return len(command)

    return sum(
        _best_arrowpad_move_len(start, end, target_depth, depth + 1)
        for start, end in zip(f"A{command}", command)
    )


@functools.cache
def _best_numpad_move_len(start_symbol, end_symbol, target_arrowpad_depth):
    base_arrowpad_options = _numpad_move_options(start_symbol, end_symbol)
    return min(
        _arrowpad_command_len_for_arrowpad_command(cmd, target_arrowpad_depth)
        for cmd in base_arrowpad_options
    )


@functools.cache
def _best_arrowpad_move_len(start_symbol, end_symbol, target_depth, depth):
    base_arrowpad_options = _arrowpad_move_options(start_symbol, end_symbol)
    return min(
        _arrowpad_command_len_for_arrowpad_command(cmd, target_depth, depth)
        for cmd in base_arrowpad_options
    )
